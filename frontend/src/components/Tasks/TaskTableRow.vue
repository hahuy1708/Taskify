...
const toggleDetail = () => { openDetail.value = !openDetail.value }
</script>

<template>
  <tr class="border-b hover:bg-gray-50">
    <td class="px-6 py-4">
      <button class="text-left text-gray-900 font-medium hover:underline" @click="toggleDetail">{{ props.task.name }}</button>
    </td>
    <td class="px-6 py-4">
      <div class="flex items-center gap-2">
        <span class="text-gray-700">{{ props.task.project?.name || '—' }}</span>
        <span v-if="props.task.project?.is_personal" class="text-[10px] px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">Personal</span>
      </div>
    </td>
    ...
    <td class="px-6 py-4">
      <span
        ...
        :class="{
          'bg-gray-100 text-gray-700': !props.task.priority || props.task.priority === 'low',
    ...
    <td class="px-6 py-4 text-gray-700">{{ props.task.assignee?.full_name || props.task.assignee?.username || '—' }}</td>
    <td class="px-6 py-4 text-gray-700">{{ fmtDate(props.task.deadline) }}</td>
    <td class="px-6 py-4 text-right whitespace-nowrap">
      <span class="inline-flex items-center gap-2">
        <button @click="toggleDetail" class="px-3 py-1.5 text-xs rounded-lg border border-gray-300 hover:bg-gray-100" title="View detail">
      ...
      <TaskDetaiModal :open="openDetail" :task-id="props.task.id" @close="openDetail=false" />
    </td>
  </tr>
</template>
