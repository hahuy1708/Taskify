...
export const createTeam = async (projectId, name) => {
  try {
    const response = await api.post(`projects/${projectId}/teams/`, { name });
    console.log('Create team response:', response.data);
    return response.data;
  } catch (error) {
    console.error('Create team error:', error.response ? error.response.data : error.message);
    throw error;
  }
};
